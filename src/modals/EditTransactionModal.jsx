  ...
  return(
    <Sheet open={true} onClose={onClose} showCloseButton={false} footer={
      ...
      <div style={{display:"flex",flexDirection:"column",gap:14,paddingTop:22,paddingBottom:8}}>
        ...
        <div>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,letterSpacing:0.8,textTransform:"uppercase",marginBottom:6,fontFamily:"'Noto Sans',sans-serif"}}>{t(lang,"editTxType")}</div>
          <div style={{display:"flex",gap:6}}>
            {pillBtn(editType==="expense",t(lang,"editTxExpense"),()=>flipType("expense"),"rgba(255,179,167,0.3)")}
            {pillBtn(editType==="income",t(lang,"editTxIncome"),()=>flipType("income"),"rgba(172,225,175,0.3)")}
          </div>
        </div>
        <div>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,letterSpacing:0.8,textTransform:"uppercase",marginBottom:6,fontFamily:"'Noto Sans',sans-serif"}}>{t(lang,"editTxCurrency")}</div>
          <div style={{display:"flex",gap:6}}>
            {Object.entries(CURR).map(([code,c])=>pillBtn(editCurrency===code,`${c.symbol} ${code}`,()=>setEditCurrency(code)))}
          </div>
        </div>
        <div>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,letterSpacing:0.8,textTransform:"uppercase",marginBottom:6,fontFamily:"'Noto Sans',sans-serif"}}>{t(lang,"editTxAmount").replace("{currency}",editCurrency)}</div>
        ...
        <div>
          <div style={{fontSize:11,fontWeight:700,color:T.muted,letterSpacing:0.8,textTransform:"uppercase",marginBottom:8,fontFamily:"'Noto Sans',sans-serif"}}>{t(lang,"category_label")}</div>
