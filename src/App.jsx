    ...
    <div style={{padding:"calc(env(safe-area-inset-top, 8px) + 8px) 20px 24px",position:"relative",zIndex:1}}>
      ...
      <div style={{background:T.surface,backdropFilter:"blur(20px)",borderRadius:20,boxShadow:T.shadow,marginBottom:20,overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,padding:"16px 18px"}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:14,fontWeight:600,color:T.dark,fontFamily:"'Noto Sans',sans-serif"}}>Guest PIN (wife)</div>
...
function PinLock({ pinConfig, pinInput, pinShake, onKey, isSetup, setupMode, setupStep }) {
  ...
  const subtitle = isSetup
    ? (setupStep === "confirm" ? "Confirm your PIN" : setupMode === "set-owner"
        ? "Enter new 4-digit owner PIN"
        : "Enter a guest PIN for family access")
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:9999, background:T.bg,
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 76px)", gap:14 }}>
        {keys.map((k, i) => (
          ...
          <button key={i} onClick={() => k && onKey(k)}
            style={{
              width:76, height:76, borderRadius:22, border:"none", fontFamily:"'Noto Sans',sans-serif",
              background: k === "⌫" ? "rgba(172,225,175,0.15)" : T.surface,
              color: k === "⌫" ? T.muted : T.dark,
              fontSize: k === "⌫" ? 20 : 26, fontWeight: k === "⌫" ? 400 : 500,
      ...
      {!isSetup && pinConfig?.guest && (
        <div style={{ marginTop:28, fontSize:12, color:T.muted, textAlign:"center", fontFamily:"'Noto Sans',sans-serif" }}>
          Owner and guest PINs both accepted
        </div>
      )}
