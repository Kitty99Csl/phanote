...
export function ProUpgradeScreen({ lang = "lo", onClose }) {
  const [billing, setBilling] = useState("monthly");
  const isAnnual = billing === "annual";

  const price    = isAnnual ? "$29.99" : "$2.99";
  const period   = isAnnual ? t(lang,"proPerYear") : t(lang,"proPerMonth");
  const alts     = isAnnual ? "฿999 / year  ·  ₭699,000 / year" : "฿99 / month  ·  ₭69,000 / month";
  const ctaLabel = t(lang,"proCtaStart").replace("{price}", isAnnual ? "$29.99 "+t(lang,"proPerYear") : "$2.99 "+t(lang,"proPerMonth"));
    ...
    <div style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px",borderBottom:"0.5px solid rgba(45,45,58,0.05)"}}>
      ...
      <div style={{display:"flex",gap:18,flexShrink:0,width:60,justifyContent:"space-between"}}>
        ...
        <span style={{fontSize:14,color:pro?"#3da873":"#ddd",width:20,textAlign:"center"}}>
          {coming ? <span style={{fontSize:9,fontWeight:700,background:"rgba(172,225,175,0.2)",color:"#2A7A40",padding:"2px 5px",borderRadius:4}}>{t(lang,"proSoon")}</span> : pro?"✓":"—"}
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:600,background:"#F7FCF5",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
      ...
      <div style={{background:"#1A4020",padding:"calc(env(safe-area-inset-top,0px) + 24px) 20px 22px"}}>
        <button onClick={onClose} style={{fontSize:13,color:"rgba(255,255,255,0.5)",background:"none",border:"none",cursor:"pointer",fontFamily:"'Noto Sans',sans-serif",padding:0,marginBottom:14}}>{t(lang,"proBackSettings")}</button>
        <div style={{marginBottom:6}}><Logo size={96} /></div>
        <div style={{fontSize:21,fontWeight:800,color:"#fff",letterSpacing:-0.5,fontFamily:"'Noto Sans',sans-serif"}}>{t(lang,"proTitle")}</div>
        <div style={{fontSize:12,color:"rgba(255,255,255,0.55)",marginTop:5,lineHeight:1.6}}>{t(lang,"proSubtitle")}</div>
      </div>
      ...
      <div style={{padding:"14px 16px 0"}}>
        ...
        <div style={{display:"flex",background:"rgba(45,45,58,0.07)",borderRadius:12,padding:3,marginBottom:14}}>
          {["monthly","annual"].map(m=>(
            <button key={m} onClick={()=>setBilling(m)}
              ...
              {m==="monthly"?t(lang,"proMonthly"):<>{t(lang,"proAnnual")} <span style={{fontSize:9,fontWeight:700,background:"#ACE1AF",color:"#1A4020",padding:"1px 5px",borderRadius:4}}>-17%</span></>}
            </button>
          ))}
        </div>
        ...
        <div style={{background:"#fff",borderRadius:18,padding:"16px",border:"1.5px solid #ACE1AF",marginBottom:14}}>
          ...
          {isAnnual&&<div style={{fontSize:11,color:"#3da873",fontWeight:700,marginTop:6}}>{t(lang,"proSaveAnnual")}</div>}
        </div>
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{display:"flex",alignItems:"center",padding:"0 16px 8px"}}>
            ...
            <div style={{display:"flex",gap:18,width:60,justifyContent:"space-between"}}>
              <span style={{fontSize:10,fontWeight:700,color:"#9B9BAD",width:20,textAlign:"center"}}>{t(lang,"proColumnFree")}</span>
              <span style={{fontSize:10,fontWeight:700,color:"#1A4020",width:20,textAlign:"center"}}>{t(lang,"proColumnPro")}</span>
            </div>
          </div>

          <div style={{background:"#fff",borderRadius:18,overflow:"hidden",border:"0.5px solid rgba(45,45,58,0.07)"}}>
            <FeatRow emoji="💬" title={t(lang,"proFeatLog")}       desc={t(lang,"proFeatLogDesc")}           free pro />
            <FeatRow emoji="📊" title={t(lang,"proFeatAnalytics")} desc={t(lang,"proFeatAnalyticsDesc")}       free pro />
            <FeatRow emoji="🎯" title={t(lang,"proFeatGoals")}    desc={t(lang,"proFeatGoalsDesc")}                  free pro />
            <FeatRow emoji="🔐" title={t(lang,"proFeatPin")}  desc={t(lang,"proFeatPinDesc")}     free pro />
            <FeatRow emoji="🤖" title={t(lang,"proFeatAdvisor")}    desc={t(lang,"proFeatAdvisorDesc")}    pro />
            <FeatRow emoji="📷" title={t(lang,"proFeatOcr")}        desc={t(lang,"proFeatOcrDesc")}         pro />
            <FeatRow emoji="🧠" title={t(lang,"proFeatMemory")}          desc={t(lang,"proFeatMemoryDesc")}   pro />
            <div style={{display:"flex",alignItems:"center",gap:12,padding:"11px 16px"}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:"#2D2D3A",fontFamily:"'Noto Sans',sans-serif"}}>{t(lang,"proFeatExport")}</div>
                <div style={{fontSize:11,color:"#9B9BAD",marginTop:1}}>{t(lang,"proFeatExportDesc")}</div>
              </div>
              <div style={{display:"flex",gap:18,flexShrink:0,width:60,justifyContent:"space-between"}}>
                <span style={{fontSize:14,color:"#ddd",width:20,textAlign:"center"}}>—</span>
                <span style={{fontSize:9,fontWeight:700,background:"rgba(172,225,175,0.2)",color:"#2A7A40",padding:"2px 5px",borderRadius:4}}>{t(lang,"proSoon")}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{textAlign:"center",fontSize:11,color:"#9B9BAD",lineHeight:1.7,marginBottom:32}}>
          {t(lang,"proDisclaimer")}<br/>PromptPay · BCEL · International card
        </div>
      </div>
    </div>
  );
