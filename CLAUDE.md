...
22. **Handler contract: modal-throw vs event-no-throw.** Handlers called from modals (where caller needs to branch UX based on success/failure) should THROW on failure. Handlers called from fire-and-forget event handlers should NOT throw — the helper does revert + toast as the complete user signal. Mixing these creates either unhandled rejections (modal handler in event context) or silent failures (event handler in modal context). Sprint M.1-M.2b3 codified this across the codebase. Existing handlers matching the rule: handleUpdateCategory throws (called from EditTransactionModal); handleOnboarding, handleUpdateProfile, addSavings, performDeleteGoal, saveBudget, updateStreak, deleteBatch all do not throw (called from event handlers). Future handlers must declare their caller context in design phase, choose throw/no-throw consistently.
